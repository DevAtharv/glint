...
function GlintApp() {
  ...
  // 🛠 Navigation Handler (Fixes the TypeScript Error)
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'import': 
        return (
          <ImportPage 
            onSavePlaylist={(p) => setPlaylists(prev => [p, ...prev])} 
            onPlay={player.playTrack} 
            currentTrack={player.currentTrack} 
            onNavigate={handleNavigate} 
          />
    ...
    <div className="flex h-screen bg-[#121212] text-white overflow-hidden relative flex-col lg:flex-row">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex justify-around items-center bg-[#121212]/95 backdrop-blur-xl border-t border-white/5 py-4 pb-8 px-6">
         {['home', 'search', 'library', 'import'].map((p) => (
           <button key={p} onClick={() => handleNavigate(p as Page)} className={`flex flex-col items-center gap-1 transition-all ${page === p ? 'text-[#00e628]' : 'text-white/30'}`}>
