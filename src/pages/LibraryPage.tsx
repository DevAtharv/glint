...
export default function LibraryPage({ playlists, onPlayPlaylist, onPlay, currentTrack, onNavigate }: LibraryPageProps) {
  const { user } = useAuth()
      ...
      <main className="p-6 max-w-[1400px] mx-auto">
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
            {playlists.length > 0 ? (
              playlists.map(pl => (
                <div key={pl.id} className="group cursor-pointer" onClick={() => onPlayPlaylist(pl)}>
                  <div className="relative aspect-square rounded-xl overflow-hidden mb-3 bg-[#353535] shadow-lg border border-white/5">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button className="w-14 h-14 rounded-full bg-[#00e628] text-black flex items-center justify-center shadow-2xl translate-y-4 group-hover:translate-y-0 transition-all hover:scale-105 hover:bg-[#3be477]">
                        <Icons.PlayFill />
                      </button>
                    </div>
                  </div>
