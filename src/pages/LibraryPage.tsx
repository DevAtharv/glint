...
export default function LibraryPage({ 
  liked, playlists, onPlay, currentTrack, onLike, 
  onEditPlaylist, onPlayPlaylist, onNavigate, onDeletePlaylist 
}: LibraryPageProps) {
  return (
        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 lg:gap-6">
            {playlists.map((pl) => (
              <div 
                key={pl.id} 
                className="group relative bg-white/5 border border-white/5 rounded-3xl p-4 hover:bg-white/10 transition-all shadow-lg hover:shadow-2xl"
              >
                ...
                <div className="absolute top-6 right-6 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="bg-black/80 hover:bg-[#00e628] hover:text-black text-white p-2.5 rounded-full backdrop-blur-md transition-all shadow-lg"
                    title="Edit Playlist"
                  >
                    ✏️
                  </button>
                  <button 
                    ...
                    className="bg-black/80 hover:bg-red-500 hover:text-white text-white p-2.5 rounded-full backdrop-blur-md transition-all shadow-lg"
                ...
                <div 
                  ...
                  className="aspect-square rounded-2xl overflow-hidden mb-4 relative shadow-inner cursor-pointer"
                >
                  <img 
                ...
                <h3 className="font-bold text-white text-sm truncate px-1">{pl.name}</h3>
                <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest mt-1 px-1">{pl.tracks?.length || 0} Tracks</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="space-y-1">
          {liked.length > 0 ? (
            ...
            <div className="py-24 text-center bg-white/5 rounded-[32px] border border-dashed border-white/10">
              ...
              <button onClick={() => onNavigate('search')} className="px-8 py-3 bg-white text-black rounded-full font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-transform">
                Find Music
              </button>
            </div>
          )}
